...
import {useMediaQuery} from "@/utils/hook/useStepper";
		...
		return (
			...
			>
				<div
					...
					ref={ref}
					className={cn(
						'flex w-full justify-between gap-4 text-center',
		...
		>
			<div className={cn('flex items-center gap-2', isLabelVertical ? 'flex-col' : '')}>
				<Button
					...
					disabled={!hasVisited}
					className={cn(
						'aspect-square h-12 w-12 rounded-full data-[highlighted=true]:bg-primary data-[highlighted=true]:text-primary-foreground',
...
const StepLabel = ({
	...
	return shouldRender ? (
		...
		>
			{!!label && (
				<p className={cn('font-semibold text-muted-foreground', isCurrentStep && 'text-foreground', labelClassName)}>
					{label}
		...
		return (
			<div
				...
				className={cn(
					...
					isCompletedStep ? 'border-primary' : ''
				)}
			>
